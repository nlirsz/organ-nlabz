...
export function SmartDashboard({ products, stats, isLoading }: SmartDashboardProps) {
  const dashboardData = useMemo(() => {
    if (!products || !stats) return null;
    ...
    const categoryColors = ['#3b82f6', '#10b981', '#f59e0b', '#8b5cf6', '#ef4444', '#64748b'];
    const categoriesData = Array.from(categoriesMap.entries()).map(([name, value], index) => ({
      name,
      value: (value / totalProducts) * 100,
      color: categoryColors[index % categoryColors.length],
    }));

    // Mocked data for now, as backend doesn't provide it yet
    const monthlyTrend = [
      { month: 'Jan', spending: 1200, products: 5 },
      { month: 'Fev', spending: 800, products: 3 },
      { month: 'Mar', spending: 1500, products: 7 },
    ...
    const recentActivity = products.slice(0, 3).map(p => ({
      type: p.isPurchased ? 'purchased' : 'added',
      product: p.name,
      timestamp: new Date(p.createdAt).getTime(),
    }));

    const priceAlerts = []; // Needs real data
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card>
          ...
          <CardContent>
            <div className="space-y-3">
              {dashboardData.priceAlerts.length > 0 ? dashboardData.priceAlerts.map((alert, index) => (
              ...
              )) : <p className="text-sm text-gray-500">Nenhum alerta de preço no momento.</p>}
            </div>
          </CardContent>
        </Card>
